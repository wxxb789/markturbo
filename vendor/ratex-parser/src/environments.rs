...
fn handle_aligned(
    ...
    if let Some(ParseNode::OrdGroup { body, .. }) = args.first() {
        ...
        // markturbo: clamp. `n * 2` AlignSpecs are allocated below; AlignSpec is
        // 64 bytes, so an unclamped N is an allocation abort catch_unwind cannot
        // contain. Fail closed: a count we will not honour is an error, not a
        // silently-ignored argument.
        const MAX_ALIGNAT_COLUMNS: usize = 256;
        let trimmed = arg_str.trim();
        if !trimmed.is_empty() {
            match trimmed.parse::<usize>() {
                Ok(n) if n <= MAX_ALIGNAT_COLUMNS => {
                    num_maths = n;
                    explicit_cols = n * 2;
                }
                Ok(n) => {
                    return Err(ParseError::msg(format!(
                        "Too many columns in {}: {} exceeds the limit of {}",
                        ctx.env_name, n, MAX_ALIGNAT_COLUMNS
                    )));
                }
                Err(_) => {}
            }
